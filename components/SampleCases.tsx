...
export function SampleCases({ className, disabled = false, onSelect }: SampleCasesProps) {
  return (
    <section aria-labelledby="sample-cases-heading" className={className}>
      <div className="mb-3 flex items-center justify-between">
        <h2 id="sample-cases-heading" className="text-xs font-medium text-slate-400">
          Try a sample
        </h2>
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {sampleCases.map((sampleCase) => {
          ...
          return (
            ...
            >
              <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${sampleCase.type === "error" ? "bg-rose-400/10 text-rose-300" : "bg-amber-400/10 text-amber-300"}`}>
                <Icon size={15} />
              </span>
              <span className="min-w-0">
                <span className="block truncate text-xs font-medium text-slate-300">{sampleCase.buttonLabel}</span>
                <span className="mt-0.5 block text-[11px] capitalize text-slate-600">{sampleCase.type}</span>
              </span>
            </button>
          )
        })}
